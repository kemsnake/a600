...
- создание платежей на странице rbk_money/payment или программно
- вывод формы оплаты на странице rbk_money/payment/PID или программно
- страница управления платежами admin/config/rbk_money/payments
...
- страница настроек модуля admin/config/rbk_money/settings
...
- настройка модуля осуществляется по адресу admin/config/rbk_money/settings
...
- на странице rbk_money/payment. В этом случае платеж создается
с одним обязательным параметром суммы платежа и опциональным - описание платежа.

- с помощью функции rbk_money_payment_save($params), принимающей массив
...
- перейти на страницу rbk_money/payment/PID, где PID - ID платежа.
...
Загрузка платежа осуществляется через вызов rbk_money_payment_load($pid),
 где $pid - ID платежа.
...
Осуществляется с помощью функции rbk_money_payment_enroll($pid),
где $pid - ID платежа.
